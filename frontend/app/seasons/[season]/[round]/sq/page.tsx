...
export default function QualiStats({ params }: { params: Promise<{ season: string, round: string }>}) {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_ROUTE!}/session/results?year=${season}&gp=${round}&session=sq`).then((response) => response.json()).then((content) => {
  ...
  return (
    <div className="lg:max-w-[80%] xl:max-w-[60%] mx-auto p-8 flex flex-col gap-4">
      ...
      <div>
        <h2 className="text-xl font-semibold">Sprint {season == "2023" ? "Shootout" : "Qualifying"} Results</h2>
